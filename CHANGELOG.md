# Changelog:

## Version 0.1

### Start Menu

- The search bar takes up the entire width of the Start menu
- Added a "Recommended" section (Settings -> Personalization -> Start menu -> enable/disable "Show recently added apps")
- The "Pinned" and "Recomended" heading has been removed
- The open folder from "Pinned" has been moved to the left
- The "Phone Link" and the "User Panel" have been moved lower
- Added a hover effect similar to the TranslucentStartMenu Theme
- Slightly increased the opacity of the dark gray background for all sections
          
### Taskbar

- Slightly increased the opacity of the System Tray background
          
### Notifications Center

- Decreased the opacity of the "Control Center" background 
- Removed the notification block above the calendar
- Removed the "Focus" block below the calendar
- Changed the background opacity for better readability

## Version 0.2

### Start Menu

- Multiple bug fixes
- Removed the All Apps header text
- Adjusted the corner radius
- Added search window styling
- Removed the recent section in the phone link widget for improved privacy and security
- Added minimal variation *(Should be compatible with most resolutions regardless of DPI)*
- Added high dpi start menu variation *(Should allow phone link widget on 125% DPI displays with 1920x1080 or 1920x1200 screen resolutions)*

### Taskbar

- Multiple bug fixes
- Adjusted the corner radius
- Added a grey start button
- Should be compatible with most resolutions regarldess of DPI
- Added active highlight to task list icons.
- Added full width taskbar variant.

### Notification Center

- Adjusted the corner radius
- Updated background colors to match the rest of the theme
- Various visual tweaks and adjustments
